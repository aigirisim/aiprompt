...
class CurvedTextView @JvmOverloads constructor(
    ...
    var text: String by Delegates.observable("") { _, _, _ ->
        // When the text changes, update the path
        updatePath()
    }
    private var textColor: Int by Delegates.observable(Color.BLACK) { _, _, new ->
        // When the text color changes, update the paint
        paint.color = new
    }
    private var textSize: Float by Delegates.observable(0f) { _, _, new ->
    ...
    init {
        ...
        val text = typedArray.getString(R.styleable.CurvedTextView_text) ?: ""
        val textColor = typedArray.getColor(R.styleable.CurvedTextView_textColor, Color.BLACK)
        val textSize = typedArray.getDimension(R.styleable.CurvedTextView_textSize, 0f)
    ...
    private fun updatePath() {
        ...
        when (curveDirection) {
            0 -> { // top_to_bottom
                ...
                newPath.quadTo(centerX, centerY, centerX, height.toFloat()) // Draw a winding path
            }
            1 -> { // bottom_to_top
                // Add a curve upwards starting from the bottom of the road
                newPath.moveTo(centerX, height.toFloat()) // Set the starting point of the path
        ...
        newPath.addArc(centerX - radius, centerY - radius, centerX + radius, centerY + radius, -90f, angle)
    ...
    override fun onDraw(canvas: Canvas) {
        // Draw the text on the path, check for null
        path?.let {
            // Use the paint object to draw the text on the curved path
            canvas.drawTextOnPath(text, it, 0f, 0f, paint)
        }
    }
}